...
void chunk_new(chunk_t *chunk, blocks_t blocks) {
    ...
    arr_new_reserve(mesh_vertices, CHUNK_TOTAL * 120);
        ...
        for (size_t y = 0; y < CHUNK_SIZE; y++) {
            for (size_t z = 0; z < CHUNK_SIZE; z++) {
                ...
                for (size_t i = 0; i < 24; i++) {
                    ...
                    float vertex[5] = {
                        /* Chunk-local vertex position. */
                        cube_positions[i * 3 + 0] + x,
                        cube_positions[i * 3 + 1] + y,
                        cube_positions[i * 3 + 2] + z,

                        /* TODO: update uvs based on block type. */
                        cube_uvs[i * 2 + 0] / 16.f + uv_offs[0],
                        cube_uvs[i * 2 + 1] / 16.f + uv_offs[1]};

                    arr_append_n(mesh_vertices, 5, vertex);
                }
