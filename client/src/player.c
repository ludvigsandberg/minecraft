...
void player_new(void) {
    ...
    /* Shader */
    player_shader = shader_program_new("client/res/chunk_vs.glsl",
    ...
    /* Texture */
    bitmap_load(&data, &w, &h, "client/res/char.bmp");
    ...
    glBufferData(GL_ARRAY_BUFFER, 6 * 4 * 7 * sizeof(f32), NULL,
                 GL_DYNAMIC_DRAW);
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, 6 * 6 * sizeof(u32), NULL,
                 GL_DYNAMIC_DRAW);
...
static void upload_limb_mesh(const limb_t *limb) {
    ...
    for (face = 0; face < 6; face++) {
        int i;
        u32 base        = (u32)(vcount / 7);
        u32 face_idx[6] = {0, 1, 2, 2, 3, 0};
    ...
    glBindBuffer(GL_ARRAY_BUFFER, limb_vbo);
    glBufferSubData(GL_ARRAY_BUFFER, 0, (GLsizeiptr)(vcount * sizeof(f32)),
                    vertices);

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, limb_ebo);
    glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0,
                    (GLsizeiptr)(icount * sizeof(u32)), indices);
}

static void draw_limb(int limb_idx, const mat4x4_t *base_model, f32 rx, f32 rz,
                      const camera_t *cam) {
    ...
    if (rx != 0.0f)
        mat4x4_rotate_x(&model, &model, rx * ((f32)M_PI / 180.0f));
    if (rz != 0.0f)
        mat4x4_rotate_z(&model, &model, rz * ((f32)M_PI / 180.0f));
...
void player_draw(const vec3_t *position, f32 yaw, const vec3_t *velocity,
                 f32 dt, const camera_t *camera) {
    f32 speed;
    f32 leg_angle;
    f32 arm_angle;
    ...
    mat4x4_rotate_y(&base_model, &base_model, yaw * ((f32)M_PI / 180.0f));
